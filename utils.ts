...
const getTextParts = async (text?: string): Promise<string[]> => {
	return splitTextIntoChunks(
		text || (await Bun.file(INPUT_FILE_PATH).text()),
		MAX_CHUNK_LENGTH,
	)
}
...
const runService = async (
	data: WorkerData,
): Promise<{ index: number; result: string; msg: string }> => {
	return new Promise(
		(
			resolve: (value: AudioChunk | Promise<AudioChunk>) => void,
			reject: (reason?: ErrorEvent) => void,
		): void => {
			const worker = new Worker('./worker.ts')
			worker.onmessage = (
				msg: MessageEvent<{ index: number; result: string; msg: string }>,
			): void => {
				resolve(msg.data)
			}
...
const createRuns = (
	...
	for (let i = 0; i < THREADS; i++) {
		...
		for (let j = 0; j < chunk.length; j++) {
			promiseArray.push(
				runService({ i: i * chunkSize + j, part: chunk[j], number, sessionId }),
			)
		}
	}
...
export const TTS = async (): Promise<void> => {
	const choice = Number(askQuestion('Use sessionId from\n1: file\n2: prompt\n'))

	console.log(chalk.green('-------------------------------------'))

	const sessionId: string = await getSessionId(choice)

	const inputChoice = Number(askQuestion('Use text from\n1: file\n2: prompt\n'))
	...
	results.sort((a: AudioChunk, b: AudioChunk): number => a.index - b.index)

	const title = askQuestion('Enter file name:\n')

	for (const result of results) {
		if (result.result) {
			await appendFile(
				`output${projectNum}/${title}.mp3`,
				Buffer.from(result.result, 'base64'),
			)
		}
	}

	const resultMessages = results.map((result: AudioChunk): string => result.msg)
